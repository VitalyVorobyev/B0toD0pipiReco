...
void UISetter::SetDslInfo(std::vector<Particle> &dsl){
  for(int i=0; i<dsl.size(); i++){
    dsl[i].userInfo(DUserInfo());
    if(m_mc_flag) setMCtruth(dsl[i]);
  }
}
...
void UISetter::SetModeB0(std::vector<Particle> &pl, const int mode){
  for(int i=0; i<pl.size(); i++){
    dynamic_cast<B0UserInfo&>(pl[i].userInfo()).Mode(mode);
  }
}
...
void UISetter::SetMbc_deltaE(Particle& b0,const double& mbc,const double& de){
  dynamic_cast<B0UserInfo&>(b0.userInfo()).Mbc(mbc);
  dynamic_cast<B0UserInfo&>(b0.userInfo()).deltaE(de);
}
